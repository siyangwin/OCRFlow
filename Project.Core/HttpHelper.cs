        ...
        public HttpResult GetHtml(HttpItem item, HttpContext httpContext = null)
        {
            ...
            finally
            {
                if (httpContext != null)
                {
                    ...
                    RequestUrl.Add("/api/othersystemdatasync/product");
                    SystemLogType type;
                    if (!RequestUrl.Contains(httpContext.Request.Path))
                    {
                        //非排除的API寫入數據
                        type = SystemLogType.SqlAndLocal;
                    }
                    else
                    {
                        //排除的API寫入數據
                        type = SystemLogType.Local;
                    }
                    GlobalConfig.SystemLogService.AddLogByHttpContext(httpContext, httpContext.Request.Path, "第三方接口", "URL:" + item.URL + (item.PostdataByte != null && item.PostdataByte.Length != 0 ? Encoding.UTF8.GetString(item.PostdataByte) : ""), JsonConvert.SerializeObject(result), type, (DateTime.Now - ReqTime).ToString());
